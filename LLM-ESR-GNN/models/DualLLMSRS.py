...
class DualLLMSASRec(SASRec_seq):
    ...
    def __init__(self, user_num, item_num, device, args):
        ...
        # NEW: Project collaborative emb from 128 → hidden_size (64)
        # Linear Projector
        # self.id_projector = nn.Linear(id_item_emb.shape[1], args.hidden_size)

        # MLP projector
        # self.id_projector_out_dim = 64
        # self.id_projector = nn.Sequential(
        #                         nn.Linear(128, 128),
        #                         nn.ReLU(),
        #                         nn.LayerNorm(128),
        #                         nn.Linear(128, self.id_projector_out_dim)
        #                         )
    ...
    def _get_embedding(self, log_seqs):
        ...
        # id_seq_emb = self.id_projector(self.id_item_emb(log_seqs))
    ...
    def log2feats(self, log_seqs, positions):       # log_seq: batch_size * seq_len

        # Linear Projector
        # id_seqs = self.id_projector(self.id_item_emb(log_seqs))
        # id_seqs *= self.id_projector.out_features ** 0.5  # QKV/sqrt(D)
        ...
        # MLP projector
        # id_seqs = self.id_projector(self.id_item_emb(log_seqs))
        # id_seqs *= self.id_projector_out_dim ** 0.5
