...
class LLMESR_SASRec(DualLLMSASRec):
    ...
    def forward(self, 
        ...
        if self.args.ablation_mode == "mean":
            ...
            sim_log_feats = sim_log_feats[:, 0, :]      # use top1 neightbor to do enhancement
        elif self.args.ablation_mode == "weighted_mean":
            pass
        else:
            raise ValueError(f"Unknown ablation mode {self.args.ablation_mode}")
